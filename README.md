# Репозиторий для 3.6 PyTest — параметризация, конфигурирование, плагины
...
PS: assert disabled is None в test_items.py сделан, потому что по заданию должен быть assert. А ставить в assert find_element_by_css_selector(...) некорректно, потому что он генерирует свое исключение, а не AssertionError, которые ожидается от assert по смыслу.
